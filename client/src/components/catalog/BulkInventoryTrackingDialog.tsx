...
export function BulkInventoryTrackingDialog({ productIds, onClose, onApplied }: {
  productIds: number[];
  onClose: () => void;
  onApplied: (result: BulkInventoryTrackingResult) => void;
}) {
  ...
  const review = useMutation({
    mutationFn: async () => readResult(await fetch(`${BULK_INVENTORY_TRACKING_PATH}/preview`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ productIds, inventoryTrackingDefault: next }),
    }), bulkInventoryTrackingPreviewSchema),
    onMutate: () => { setError(null); setPreview(null); setCommandKey(null); },
  ...
  const apply = useMutation({
    mutationFn: async () => {
      ...
      return readResult(await fetch(`${BULK_INVENTORY_TRACKING_PATH}/apply`, {
        ...
        body: JSON.stringify({ productIds, inventoryTrackingDefault: preview.inventoryTrackingDefault, expectedPreviewHash: preview.previewHash }),
    ...
    <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto" onInteractOutside={event => { if (busy) event.preventDefault(); }}>
      ...
      <div className="space-y-2">
        ...
        <Select value={String(next)} disabled={busy} onValueChange={value => {
          setNext(value === "true"); setPreview(null); setCommandKey(null); setError(null);
        }}>
      ...
      {preview && <div className="space-y-3" data-testid="bulk-inventory-review">
        ...
        {blockedCount > 0 && <p role="alert" className="text-sm text-destructive">{blockedCount} products have blockers. No products will change. Close this review to adjust your selection or resolve the blockers.</p>}
        <ul className="max-h-80 overflow-y-auto divide-y rounded-md border">
          {preview.products.map(product => <li key={product.productId} className="p-3 space-y-1" data-testid={`bulk-inventory-product-${product.productId}`}>
            ...
            {product.blockers.map((blocker, index) => <p key={`${blocker.variantId}-${blocker.code}-${index}`} className="text-xs text-destructive">{blocker.message}</p>)}
          </li>)}
        </ul>
      </div>}
      <DialogFooter className="gap-2">
        <Button variant="outline" disabled={busy} onClick={onClose}>Cancel</Button>
        <Button variant={preview ? "outline" : "default"} disabled={busy} onClick={() => review.mutate()}>{review.isPending ? "Reviewing…" : preview ? "Refresh review" : "Review changes"}</Button>

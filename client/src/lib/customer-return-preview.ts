...
export interface PreviewSelectionDraft {
  lineId: string;
  quantity: string;
  reasonCode: ReturnPreviewReason | null;
}
export interface PreviewParcelDraft {
  key: number;
  items: { lineId: string; quantity: string }[];
}
export type PreviewSelections = ReturnPreviewReviewInput["selections"];
...
export function describePreviewItem(
  ...
  if (index < 0) {
    throw new Error(
      "The sample item could not be identified. Find the order again.",
    );
  }
...
export function buildPreviewReviewInput(
...
): PreviewValidation<ReturnPreviewReviewInput> {
  const totals = new Map<string, number>();
  const parcelInputs: ReturnPreviewReviewInput["parcels"] = [];
  for (const [index, parcel] of parcels.entries()) {
    const items: ReturnPreviewReviewInput["parcels"][number]["items"] = [];
    const seen = new Set<string>();
  ...
  const parsed = returnPreviewReviewInputSchema.safeParse({
...
export async function readPreviewResponse<T extends z.ZodTypeAny>(
  ...
  if (!parsed.success)
    throw new Error(
      "The preview response could not be verified. Please try again.",
    );
  return parsed.data;
}

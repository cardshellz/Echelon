...
import { z } from "zod";
import { pricingProfileStateSchema, pricingReviewResponseSchema, pricingTargetsResponseSchema,
...
type Scope = PricingProfile["groups"][number]["scope"];
function ScopePicker({ endpoint, value, onChange, disabled }: { endpoint: string; value: Scope; onChange: (value: Scope) => void; disabled: boolean }) {
  ...
  const query = useQuery({ queryKey: [endpoint, "targets", value.type, queryText, page], retry: false,
    queryFn: async () => pricingTargetsResponseSchema.parse(await fetchJson(`${endpoint}/targets?type=${value.type}&search=${encodeURIComponent(queryText)}&page=${page}`)) });
  ...
  return <div className="space-y-2"><div className="grid gap-3 sm:grid-cols-2">
    ...
    <div className="flex items-center justify-between text-xs text-zinc-500"><span>{selected.filter((id) => id !== "" && id !== "0").length} selected across searches</span>
      <div className="flex gap-2"><Button type="button" size="sm" variant="ghost" disabled={disabled || page === 0} onClick={() => setPage(page - 1)}>Previous choices</Button>
        <Button type="button" size="sm" variant="ghost" disabled={disabled || !query.data || (page + 1) * PRICING_REVIEW_PAGE_SIZE >= query.data.total} onClick={() => setPage(page + 1)}>Next choices</Button></div></div>
  </div>;
}

import { DailyBuyingReview } from "@/features/purchasing/DailyBuyingReview";
...
export default function PurchasingDashboard() {
  ...
  if (isLoading || !data) {
    return (
      <div className="space-y-6 overflow-y-auto p-4 md:p-6"><section className="rounded-lg border bg-card p-5"><h1 className="text-xl font-semibold">Purchasing Dashboard</h1><h2 className="mt-4 text-lg font-semibold">Daily buying review</h2><p role={isLoading ? "status" : "alert"} className="mt-2 text-sm text-muted-foreground">{isLoading ? "Loading buying priorities…" : "Buying priorities could not be loaded. Open the Reorder Engine to review current recommendations."}</p><Button className="mt-3" onClick={() => navigate("/reorder-analysis")}>Review recommendations</Button></section><PurchasePipeline /></div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-10 px-4 py-3 md:px-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Buying decisions and purchase progress · {today}</p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-7">
        <DailyBuyingReview data={data} useNewPoEditor={useNewPoEditor} />
        <PurchasePipeline />
        <details className="rounded-lg border bg-card">
          <summary className="cursor-pointer px-4 py-4 text-sm font-semibold">
            Planning and system details
            {procurementHealth && (procurementHealth.critical > 0 || procurementHealth.warning > 0) && (
              <span className="ml-3 inline-flex flex-wrap gap-2 text-xs font-medium">
                {procurementHealth.critical > 0 && <span className="rounded-full bg-red-100 px-2 py-1 text-red-800 dark:bg-red-950 dark:text-red-300">{procurementHealth.critical} critical</span>}
                {procurementHealth.warning > 0 && <span className="rounded-full bg-amber-100 px-2 py-1 text-amber-800 dark:bg-amber-950 dark:text-amber-300">{procurementHealth.warning} warnings</span>}
              </span>
            )}
            {!procurementHealth && <span className="ml-3 text-xs font-normal text-muted-foreground">Health status unavailable</span>}
          </summary>

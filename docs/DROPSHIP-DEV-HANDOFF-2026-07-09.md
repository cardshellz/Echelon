...
| 0.8 | Implemented in PR #909: dropship uses the shared cartonizer v3 for real 3D placement, all six rotations, multi-carton splitting, and a 50 lb packed-carton handling ceiling. Maximum units/package is no longer used. Production quote verification remains. |
...
**0.8 Shared 3D cartonizer (PR #909).** Dropship now delegates physical packing to `server/modules/shipping-engine/domain/cartonize.ts`, including co-packing compatible SKUs from the same order. Cartonizer v3 expands ordered quantity into physical units, tries all six orthogonal rotations, records coordinates and rotated dimensions, rejects overlap and out-of-bounds placement, and splits across cartons when geometry or weight requires it. Dropship quote payload v2 preserves the carton item list and placement plan for audit and later packing UI. A long item cannot fit a short cube merely because its total volume is smaller. Every packed carton is capped at 22,679 g (just under 50 lb); a box-specific `max_weight_grams` may impose a lower structural limit but is not required. `max_units_per_package` is deprecated compatibility data and is ignored by quote cartonization. Rider/void consolidation defaults off until void dimensions can be modeled physically. (§3.6.)

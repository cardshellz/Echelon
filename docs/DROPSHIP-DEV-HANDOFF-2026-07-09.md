...
| 0.8 | Implemented in PR #909 plus rollout-safety follow-up: standalone cartonizer v3.1 provides real 3D placement, all six rotations, persisted WMS pack plans, multi-carton splitting, and a 50 lb packed-carton handling ceiling. Dropship is one adapter. WMS testing is explicit or opt-in shadow only; it defaults off and never gates fulfillment. Maximum units/package is no longer used. Production shadow verification remains. |
...
**0.8 Standalone 3D cartonizer (PR #909 + rollout-safety follow-up).** Physical packing now lives in `server/modules/cartonization`, not in dropship. Cartonizer v3.1 expands ordered quantity into physical units, tries all six orthogonal rotations, records coordinates and rotated dimensions, rejects overlap and out-of-bounds placement, and splits across cartons when geometry or weight requires it. The WMS adapter can persist placements through an explicit per-order endpoint or opt-in shadow execution; neither path gates `ready_to_ship`, changes status, or routes failures to `exception`. Dropship quote payload v2 preserves the same result for audit. A long item cannot fit a short cube merely because its total volume is smaller. Every packed carton is capped at 22,679 g (just under 50 lb); a box-specific `max_weight_grams` may impose a lower structural limit but is not required. `max_units_per_package` is deprecated compatibility data and is ignored. Rider/void consolidation defaults off until void dimensions can be modeled physically. (§3.6.)

Production data result, July 13: profile `COGS-TEST-001-P1` still contains legacy `max_units_per_package=4` and active box `8X6X4` has `max_weight_grams=NULL`. Neither is a blocker after PR #909. Before `SHIPCFG-04`/`SHIPCFG-08`, verify the SKU's canonical weight and all three dimensions, the box's inner dimensions and tare, and the actual multi-unit quote result.

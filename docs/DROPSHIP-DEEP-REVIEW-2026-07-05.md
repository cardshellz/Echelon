...
Resolution, July 13: dropship now uses shared cartonizer v3. It expands quantities into physical units, tests non-overlapping placement with all six rotations, records coordinates/orientation, and splits cartons on geometry or packed weight. A mandatory `max_units_per_package` guard is no longer used. NULL box `max_weight_grams` uses the automatic 22,679 g handling ceiling; a lower box-specific structural limit remains optional. Production verification still belongs in `SHIPCFG-03A`, `SHIPCFG-04`, and `SHIPCFG-08`.

...
| Recommendation policy and receiving authority | The analysis screen carries the saved planning policy and revision into the engine. Forecast and automation settings resolve the canonical warehouse row, and saves update only that row transactionally. The [receive-selection contract](../shared/procurement/purchase-receive-selection.ts) records a receiving variant only when the highest active level is unique; ambiguous configurations remain visible and cannot authorize new automatic PO or RFQ creation. Existing RFQ retries retain their original draft identity. |

...
4. Every positive physical hold identifies exact owner, warehouse, level, variant and lot. Customer allocations plus retained independent build holds exhaust every reserved and picked lot unit exactly. Several orders over several lots require explicit verified assignments, not an inferred FIFO assignment. Recorded reservation counters and physically held stock are separate facts. The default strict promise handoff and the explicitly selected current-custody basis below have different proof requirements; neither infers physical ownership from a counter difference.
...
Final commit recaptures under the existing fence, validates review/impact hashes, preserves the opening provenance, applies the selected policy's separately reviewed counter handoff through the inventory owner, imports claims and original picked costs, and retains the existing atomic activation/publication behavior. Strict promise releases and current-basis translations cannot overlap. A late failure rolls back counter handoffs, new claims, fresh reservations, authority and publication together. The separately saved historical audit remains available; the failed attempt does not rewrite it.
...
Current-basis regressions additionally cover mixed bins, preserved shortages and independent build holds, no-current-order positions, explicit opt-in/blank worksheet fields, invalid physical totals, missing ownership/cost evidence, unresolved current receipt blockers, changed projection inputs and required receipt audit lineage. PostgreSQL tests exercise missing fence, changed immutable proof, changed raw counters/lots, malformed and duplicate proposals, rollback after a late final-commit failure, and concurrent idempotent replay with exactly one translation journal.

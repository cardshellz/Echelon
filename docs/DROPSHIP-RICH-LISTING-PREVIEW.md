...
- Show the vendor's current Shellz Club `.ops` plan price for the exact sellable variant. The preview no longer reads a generic channel discount. See [the corrected source contract](DROPSHIP-OPS-PRODUCT-COST.md).
...
- Use existing applicable Card Shellz rates and fee rules. The customer shipping-estimate response contains only the final shipping charge, currency, requested scenario, timestamp, and allowlisted customer messages. Rate-table identity, markup, insurance pool, dunnage, warehouse/package internals, and raw provider diagnostics remain server-side. Do not estimate marketplace charges.
...
- [ ] Connect order acceptance to the same `.ops` product-cost authority and prove wallet-charge parity before live orders. The initial preview matched the older channel-discount implementation; that was the wrong source for `.ops` prices.
...
- **Product-cost authority:** `PgShellzClubProductCostAdapter.loadProductCosts` reads the exact entitlement-linked plan, Shopify variant identity, collection exclusions, active price overrides, and configured fallback rules in a read-only consistent snapshot. `buildDropshipListingEconomics` uses those resolved cents directly; neither the listing-price override nor a channel-wide discount sets product cost. This is a current reference, not a locked acceptance snapshot.

...
| [ ] | STOP-10 | Every active dogfood box has maximum loaded weight and every active test-SKU override has maximum units/package. | `COGS-TEST-001-P1` = 4 units/package; box `8X6X4` max loaded weight is NULL | Set and verify box maximum loaded weight before quote or listing testing |
...
| [ ] | 5. Shipping config | Package, rate, markup, insurance, and return policies cover the test SKU. | Set box `8X6X4` maximum loaded weight, then complete package editor and final quote validation |
...
| [ ] | SHIPCFG-03A | Confirm the active box has a maximum loaded weight and the active test-SKU override has maximum units/package. | `COGS-TEST-001-P1` = 4 units/package; `8X6X4.max_weight_grams` is NULL | Set a verified maximum loaded weight for `8X6X4` and confirm the Shipping Config warning clears |
...
| EX-009 | P2 | Phase 5 / cartonization |  | Current shipping setup is too simplistic for real product/box optimization. | Standalone cartonization engine chooses box(es), orientations, and packing based on ordered SKUs and dimensions; usable by dropship and other channels. | Item 0.8 bounds active box weight and units/package and surfaces legacy gaps; this is only an interim guardrail for the first dogfood SKU. | Design/build plug-and-play cartonization service. | Architecture/engineering | Open |

...
| EX-009 | P2 | Phase 5 / cartonization | PR #909 + follow-up | Current shipping setup was too simplistic for real product/box optimization. | Standalone cartonization engine chooses box(es), orientations, and packing based on ordered SKUs and dimensions; usable by every physical WMS order after a measured rollout. | Standalone cartonizer v3.1 performs non-overlapping 3D placement with six rotations, persists coordinates/orientations on the WMS pack plan, co-packs compatible SKUs, and splits on geometry or weight. Dropship and checkout are adapters, not owners. Maximum units/package is ignored. Rider/void consolidation remains off until void dimensions are modeled. WMS enforcement is intentionally absent: explicit plan generation and opt-in shadow observation cannot block or change fulfillment status. | Verify one-unit, multi-unit, mixed-SKU, long-item rejection, and packed-weight behavior through the explicit endpoint. Then measure shadow verified-plan/no-fit rates and predicted-vs-actual cartons before proposing a separate enforcement PR. | Architecture/engineering | Built / shadow verification |
...
| EX-016 | P3 | Phase 14 / return inventory verification | `Inventory History` | The search field filters only the 50 transactions already loaded for the current page, so an SKU search cannot reliably find older ledger evidence. | SKU search is executed by the server across the selected date range and pagination reflects the filtered result set. | `InventoryHistory.tsx` sends date/type/location/page parameters without the search term, then filters the returned page in the browser. | Add a validated server-side SKU/search query, indexed repository filtering, filtered-result pagination, and focused API/UI tests. | Frontend/backend | Open |
| EX-017 | P3 | Phase 2 / eBay OAuth verification UX | `marz_cards`, PR #1263, PR #1265 | The email verification-code field has no adjacent submit action, so the operator must guess that the lower store-card button completes verification and starts OAuth. | The verification panel names the target store and intent and provides an explicit `Verify and refresh the eBay connection` action beside the code; Enter submits the same action. | Reauthorization succeeded only after entering the code and returning to the separate purple `Refresh Ebay connection` button. | Keep the pending store ID and OAuth intent with the challenge; add an adjacent explicit submit button, Enter-key support, pending/error feedback, and focused interaction tests. | Frontend | Open |

-- Commercial fulfillment follows exact label contents before carrier-created
-- split rows exist. Existing historical split evidence remains supported.
-- This changes validation only; it performs no operational data backfill.
...
BEGIN
  ...
  IF intent.package_allocation_package_binding_id IS NOT NULL THEN
    -- The immutable plan records the accepted package portion. The live label
    -- and its append-only provider observation independently name the same
    -- source item and quantity; SKU similarity cannot satisfy this constraint.
    ...
    IF package_portion_proven IS DISTINCT FROM TRUE THEN
      -- Compatibility for plans created with exact historical WMS split proof.
      SELECT TRUE INTO package_portion_proven
      FROM wms.package_allocation_package_bindings AS binding
      JOIN wms.shipping_provider_labels AS label

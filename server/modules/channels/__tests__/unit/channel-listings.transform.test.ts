...
describe("deriveListingStatus", () => {
  it("returns 'error' when sync_status is error, regardless of external id", () => {
    expect(deriveListingStatus("error", "62783080038559")).toBe("error");
    expect(deriveListingStatus("error", null)).toBe("error");
  });

  it("returns 'active' when external id is present and sync_status is not pending/error", () => {
    expect(deriveListingStatus("synced", "62783080038559")).toBe("active");
    expect(deriveListingStatus(null, "62783080038559")).toBe("active");
  });

  it("returns 'pending' when sync_status is pending even with an external id", () => {
    expect(deriveListingStatus("pending", "62783080038559")).toBe("pending");
  });

  it("returns 'pending' when no external id is present", () => {
    expect(deriveListingStatus("synced", null)).toBe("pending");
    expect(deriveListingStatus(null, null)).toBe("pending");
  });
});

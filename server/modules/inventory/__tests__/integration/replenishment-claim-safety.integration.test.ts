...
dbDescribe("Replenishment canonical-claim safety / legacy compatibility", () => {
  ...
  beforeAll(async () => {
    ...
    const taskColumns = getTableConfig(replenTasks).columns;
    await context.pool.query(`CREATE TABLE inventory.replen_tasks (${taskColumns.map(column => `"${column.name}" ${column.getSQLType()}`).join(",")})`);
    ...
    for (const table of ["warehouse.warehouses", "warehouse.warehouse_locations", "catalog.products", "catalog.product_variants", "inventory.replen_tasks"]) {

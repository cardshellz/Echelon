...
export async function readInventoryCutoverReconstruction(client: PoolClient): Promise<Pick<CutoverReconstructionEvidence,
  ...
  const journals = await bounded(`SELECT order_id AS "orderId", order_item_id AS "orderItemId",
    product_variant_id AS "productVariantId", position.location_id AS "warehouseLocationId",
    ...
    count(*) FILTER (WHERE (transaction_type IN ('reserve','unreserve','pick') AND reserved_qty_delta IS NULL)
      ...
      OR (transaction_type='ship' AND source_state IS DISTINCT FROM 'picked')
      OR transaction_type='reserve_move')::text AS "unknownCount",
    ...
    -- A legacy reserve_move has no exact owner delta. Keep the original row in
    -- BOTH positions' evidence; never infer ownership from its moved quantity.
    CROSS JOIN LATERAL (SELECT DISTINCT unnest(CASE WHEN transaction_type='reserve_move'
      THEN ARRAY[from_location_id,to_location_id] ELSE ARRAY[COALESCE(to_location_id,from_location_id)] END) AS location_id) position
    WHERE voided_at IS NULL AND transaction_type IN ('reserve','unreserve','pick','unpick','ship','reserve_move')
      AND COALESCE(reference_type,'') NOT LIKE 'availability_claim%'
    GROUP BY order_id, order_item_id, product_variant_id, position.location_id
    ORDER BY order_id NULLS FIRST,order_item_id NULLS FIRST,product_variant_id NULLS FIRST,position.location_id NULLS FIRST
    LIMIT $1`);

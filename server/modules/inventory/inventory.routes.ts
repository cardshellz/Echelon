...
export function registerInventoryRoutes(app: Express) {
  ...
  app.post("/api/inventory/convert-sku", requirePermission("inventory", "adjust"), async (req, res) => {
    try {
      ...
      const convertQty = quantity ? parseInt(String(quantity)) : undefined;
      if (convertQty !== undefined && (isNaN(convertQty) || convertQty <= 0)) {
      ...
      const result = await inventoryCore.convertSku({
        ...
        locationId: locId,
        quantity: convertQty,
        notes: notes ? `SKU conversion: ${fromVariant.sku} → ${toVariant.sku}. ${notes}` : undefined,
        userId,
      });

...
import { createShopifyLabelLifecycleRepository } from "../../../oms/shopify-label-lifecycle.repository";
import { createShopifyLabelLifecycleService } from "../../../oms/shopify-label-lifecycle.service";
...
describeWithDisposableDb("Package allocation ledger PostgreSQL guarantees", () => {
  ...
  it.each(['sequential', 'concurrent', 'repeated', 'rollback', 'late_void', 'ambiguous', 'competing'])(
    'matches split relabels by package lineage and updates exact Shopify quantities: %s', async mode => {
      ...
      if (mode === 'ambiguous') {
        // Provider order identities changed for all four units, but all four
        // predecessors are voided and the complete new cohort conserves the
        // same canonical source. There is no arbitrary one-to-one pairing.
        expect(await replacements[0].receive()).toMatchObject({ outcome: 'replaced' });
        expect((await pool.query('SELECT id FROM wms.physical_shipment_item_quantity_adjustments')).rowCount).toBe(4);
        expect((await pool.query("SELECT COUNT(DISTINCT replacement_batch_id)::int AS batches FROM wms.ebay_label_replacement_work WHERE state = 'applied'")).rows).toEqual([{ batches: 1 }]);
        await worker.runDueBatch();
        expect(packages.filter(pkg => pkg.status === 'SUCCESS').map(pkg => pkg.tracking).sort())
          .toEqual(replacements.slice(0, 4).map(pkg => `TRACK${pkg.providerLabelId}`).sort());
        expect(await replacements[4].receive()).toMatchObject({ outcome: 'replaced' });
        await worker.runDueBatch();
        expect(packages.filter(pkg => pkg.status === 'SUCCESS').map(pkg => pkg.tracking).sort())
  ...
  it.each(['unsent', 'sent', 'late_void', 'missed_void_poll', 'missed_void_webhook', 'split', 'lost_cancel_response', 'claimed_before_void', 'concurrent', 'receipt_rollback', 'contents_changed'])(
    'corrects Shopify void/relabel end to end without changing a sibling: %s', async mode => {
      ...
      const channelId = Number((await pool.query('SELECT channel_id FROM oms.oms_orders')).rows[0].channel_id);
      const warehouse = (await pool.query(`INSERT INTO warehouse.warehouses(code, name, shopify_location_id)
        VALUES ('LABEL-LIFECYCLE', 'Label lifecycle test', '640010') RETURNING id`)).rows[0].id;
      await pool.query(`UPDATE wms.orders SET channel_id = $1, warehouse_id = $2, external_order_id = 'gid://shopify/Order/640001'`, [channelId, warehouse]);
      await pool.query('UPDATE wms.outbound_shipments SET channel_id = $1', [channelId]);
      await seedCanonicalRequestForSource(pool, sourceId);
      const logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
      let currentTime = new Date('2099-09-21T12:00:00Z');
      const clock = { now: () => new Date(currentTime) };
      const handler = new PackageAllocationLabelCommercialFulfillmentService({ enabled: true, logger,
        workflow: createPackageAllocationLabelCommercialWorkflow({ pool, clock, logger }),
        labelLinker: { reconcileShipStationLabel: vi.fn().mockResolvedValue({ linksInserted: 0, totalLinks: 0 }) },
        reviewRepository: { record: vi.fn() },
      });
      async function label(providerLabelId: string) {
        const items = [{ lineItemKey: `wms-item-${sourceId}`, quantity: 1 }];
        const id = await seedAuthorityReadinessLabel(pool, sourceId, { providerLabelId, providerOrderId: '99001', carrierCode: 'ups',
          trackingNumber: `TRACK${providerLabelId}`, contentsLines: items, receivedAt: '2026-09-17T12:00:00Z' });
        await pool.query("UPDATE wms.shipping_provider_labels SET carrier = 'ups' WHERE id = $1", [id]);
        await pool.query(`INSERT INTO wms.shipping_provider_label_links(shipping_provider_label_id, legacy_wms_shipment_id)
          SELECT $1, shipment_id FROM wms.outbound_shipment_items WHERE id = $2`, [id, sourceId]);
        return { id, receive: () => handler.process({ shipmentId: Number(providerLabelId), orderId: 99001,
          trackingNumber: `TRACK${providerLabelId}`, isReturnLabel: false, shipmentItems: items }, { shippingProviderLabelId: String(id) } as any) };
      }
      const old = await label('44010');
      if (split) await label('44012');
      expect(await old.receive()).toMatchObject({ outcome: 'activated' });
      if (split) {
        // Another valid order line shares the legacy header but is absent from
        // these labels. Neither replacement nor carrier replay may allocate it.
        await pool.query(`WITH source AS (
          SELECT item.shipment_id, item.product_variant_id, order_item.order_id, order_item.sku, line.order_id AS oms_order_id
          FROM wms.outbound_shipment_items item
          JOIN wms.order_items order_item ON order_item.id = item.order_item_id
          JOIN oms.oms_order_lines line ON line.id = order_item.oms_order_line_id WHERE item.id = $1
        ), new_line AS (
          INSERT INTO oms.oms_order_lines(order_id, external_line_item_id, fulfillment_provider, paid_quantity, authority_fulfillable_quantity)
          SELECT oms_order_id, 'gid://shopify/LineItem/640003', 'shopify', 1, 1 FROM source RETURNING id
        ), new_item AS (
          INSERT INTO wms.order_items(order_id, oms_order_line_id, sku, quantity)
          SELECT source.order_id, new_line.id, source.sku, 1 FROM source CROSS JOIN new_line RETURNING id
        ) INSERT INTO wms.outbound_shipment_items(shipment_id, order_item_id, shipment_item_purpose, product_variant_id, qty)
          SELECT source.shipment_id, new_item.id, 'customer_fulfillment', source.product_variant_id, 1
          FROM source CROSS JOIN new_item`, [sourceId]);
      }
      const packages: Array<{ id: string; status: string; tracking: string; quantity: number }> = [];
      let lostCancelResponse = false;
      const providerCalls: Array<{ operation: string; id: string }> = [];
      const client: ShopifyAdminGraphQLClient = { async request<T>(query: string, variables?: Record<string, unknown>): Promise<T> {
        if (query.includes('labelLifecycleLines')) {
          const pkg = packages.find(item => item.id === variables?.id)!;
          return { fulfillment: { id: pkg.id, order: { id: 'gid://shopify/Order/640001' }, fulfillmentLineItems: {
            nodes: [{ quantity: pkg.quantity, lineItem: { id: 'gid://shopify/LineItem/640002' } }],
            pageInfo: { hasNextPage: false, endCursor: null },
          } } } as T;
        }
        if (query.includes('cancelVoidedLabelPackage')) {
          const pkg = packages.find(item => item.id === variables?.id)!;
          pkg.status = 'CANCELLED'; providerCalls.push({ operation: 'cancel', id: pkg.id });
          if (mode === 'lost_cancel_response' && !lostCancelResponse) { lostCancelResponse = true; throw new Error('Lost cancellation response'); }
          return { fulfillmentCancel: { fulfillment: { id: pkg.id, status: pkg.status }, userErrors: [] } } as T;
        }
        if (query.includes('exactFulfillmentPackageForOrder') || query.includes('labelLifecyclePackages')) {
          expect(variables?.id).toBe('gid://shopify/Order/640001');
          return { order: { id: 'gid://shopify/Order/640001', fulfillmentsCount: { count: packages.length }, fulfillments: packages.map(pkg => ({
            id: pkg.id, status: pkg.status, trackingInfo: [{ number: pkg.tracking }],
            fulfillmentLineItems: { nodes: [{ quantity: pkg.quantity, lineItem: { id: 'gid://shopify/LineItem/640002' } }], pageInfo: { hasNextPage: false } },
          })) } } as T;
        }
        if (query.includes('fulfillmentOrders(first:')) return { order: { fulfillmentOrders: { edges: [{ node: {
          id: 'gid://shopify/FulfillmentOrder/640020', status: 'OPEN', assignedLocation: { location: { id: 'gid://shopify/Location/640010' } },
          lineItems: { edges: [{ node: { id: 'gid://shopify/FulfillmentOrderLineItem/640021', sku: 'SHOPIFY-RELABEL',
            lineItem: { id: 'gid://shopify/LineItem/640002' }, remainingQuantity: (split ? 2 : 1) - packages.filter(pkg => pkg.status === 'SUCCESS').length } }] },
        } }] } } } as T;
        if (query.includes('fulfillmentCreateV2')) {
          const input = variables!.fulfillment as { trackingInfo: { number: string }; lineItemsByFulfillmentOrder: Array<{ fulfillmentOrderLineItems: Array<{ quantity: number }> }> };
          expect(input.lineItemsByFulfillmentOrder.flatMap(item => item.fulfillmentOrderLineItems).map(item => item.quantity)).toEqual([1]);
          const pkg = { id: `gid://shopify/Fulfillment/${640030 + packages.length}`, status: 'SUCCESS', tracking: input.trackingInfo.number, quantity: 1 };
          packages.push(pkg); providerCalls.push({ operation: 'create', id: pkg.tracking });
          return { fulfillmentCreateV2: { fulfillment: { id: pkg.id }, userErrors: [] } } as T;
        }
        throw new Error(`Unexpected provider operation: ${query.slice(0, 80)}`);
      } };
      const clients = { shopify: vi.fn(async (requestedChannel: number) => {
        expect(requestedChannel).toBe(channelId);
        return { channelId, connectionId: 987, externalAccountId: 'origin-store.myshopify.com', client };
      }), ebay: vi.fn(async () => { throw new Error('No eBay account may be used'); }) };
      const runExclusive = async <T>(id: number, action: () => Promise<T>): Promise<T | null> => {
        const client = await pool.connect();
        let locked = false;
        try {
          locked = (await client.query('SELECT pg_try_advisory_lock($1::bigint) AS locked', [id])).rows[0].locked;
          return locked ? await action() : null;
        } finally {
          if (locked) await client.query('SELECT pg_advisory_unlock($1::bigint)', [id]);
          client.release();
        }
      };
      const lifecycleRepository = createShopifyLabelLifecycleRepository(getTestDb());
      ...
      const worker = createChannelFulfillmentAuthorityService({ repository, providerExecutor: executor,
        projector: createChannelFulfillmentProjector(getTestDb()), shopifyLabelLifecycle: lifecycle, clock, logger });
      const claimed = mode === 'claimed_before_void'
        ? await repository.claimCommands({ now: clock.now(), limit: 25, leaseToken: 'before-void', leaseDurationMs: 120000 }) : [];
      if (mode !== 'unsent' && mode !== 'claimed_before_void') {
        expect(await worker.runDueBatch(), JSON.stringify(logger.error.mock.calls)).toMatchObject({ succeeded: split ? 2 : 1, reviewRequired: 0, retryScheduled: 0 });
      }
      const replacement = await label('44011');
      ...
      if (mode === 'missed_void_poll' || mode === 'missed_void_webhook') {
        ...
        const originalSnapshot = { shipmentId: 44010, orderId: 99001, orderKey: 'provider-order-key-99001', orderNumber: '640001',
          ...
          shipmentItems: [{ lineItemKey: `wms-item-${sourceId}`, sku: 'SHOPIFY-RELABEL', quantity: 1 }] };
        ...
        const scan = createShipStationLabelReconciliationService({
          ...
          source: { isConfigured: () => true,
            getLabel: async () => originalSnapshot,
      ...
      await pool.query(`INSERT INTO wms.shipping_provider_label_events(shipping_provider_label_id, event_hash, event_type,
        label_status, tracking_number, provider_occurred_at, received_at, sanitized_payload)
        VALUES ($1, $2, 'label_voided', 'voided', $3, '2026-09-17T13:00:00Z', '2026-09-17T13:00:00Z', $4)`, [old.id, hash, payload.trackingNumber, payload]);
      await pool.query("UPDATE wms.shipping_provider_labels SET label_status = 'voided', last_observed_at = '2026-09-17T13:00:00Z' WHERE id = $1", [old.id]);
      await lifecycle.observe(old.id); await lifecycle.observe(old.id);
      }
      if (claimed.length) {
        await expect(executor.execute(claimed[0])).rejects.toMatchObject({ code: 'PACKAGE_LABEL_INACTIVE' });
        await repository.completeAttempt({ commandId: claimed[0].id, leaseToken: claimed[0].leaseToken,
          outcome: 'review_required', errorCode: 'PACKAGE_LABEL_INACTIVE', startedAt: clock.now(), completedAt: clock.now() });
      }
      expect(await replacement.receive()).toMatchObject({ outcome: 'waiting', reason: 'shopify_predecessor_void_pending' });
      if (mode === 'contents_changed') {
        packages[0].quantity = 2;
        await lifecycle.runDueBatch();
        expect(providerCalls.filter(call => call.operation === 'cancel')).toHaveLength(0);
        expect((await pool.query('SELECT state FROM oms.shopify_label_void_work')).rows).toEqual([{ state: 'review' }]);
        expect((await pool.query("SELECT details->>'reasonCode' AS reason FROM wms.reconciliation_exceptions WHERE details->>'operation' = 'label_void'")).rows)
          .toEqual([{ reason: 'SHOPIFY_VOID_CONTENTS_CHANGED' }]);
        expect(await replacement.receive()).toMatchObject({ outcome: 'review', reason: 'shopify_predecessor_void_requires_review' });
        return;
      }
      if (mode === 'receipt_rollback') {
        await pool.query(`CREATE FUNCTION oms.fail_void_receipt_for_test() RETURNS trigger LANGUAGE plpgsql AS $$
          BEGIN RAISE EXCEPTION 'Injected void receipt rollback'; END; $$;
          CREATE TRIGGER fail_void_receipt_for_test BEFORE INSERT ON oms.shopify_label_void_attempts
          FOR EACH ROW EXECUTE FUNCTION oms.fail_void_receipt_for_test()`);
        await lifecycle.runDueBatch();
        expect((await pool.query('SELECT state, attempt_count FROM oms.shopify_label_void_work')).rows).toEqual([{ state: 'pending', attempt_count: 0 }]);
        expect((await pool.query('SELECT id FROM oms.shopify_label_void_attempts')).rowCount).toBe(0);
        await pool.query('DROP TRIGGER fail_void_receipt_for_test ON oms.shopify_label_void_attempts; DROP FUNCTION oms.fail_void_receipt_for_test()');
      }
      if (mode === 'concurrent') await Promise.all([worker.runLabelLifecycleBatch!(), worker.runLabelLifecycleBatch!()]);
      else await worker.runLabelLifecycleBatch!();
      await worker.runDueBatch();
      if (mode === 'lost_cancel_response') {
        expect((await pool.query('SELECT state FROM oms.shopify_label_void_work')).rows).toEqual([{ state: 'pending' }]);
        currentTime = new Date(currentTime.getTime() + 61_000); await worker.runLabelLifecycleBatch!(); await worker.runDueBatch();
      }
      expect((await pool.query('SELECT state FROM oms.shopify_label_void_work')).rows, JSON.stringify(logger.error.mock.calls)).toEqual([{ state: 'complete' }]);
      expect(await replacement.receive(), JSON.stringify(logger.error.mock.calls)).toMatchObject({ outcome: 'replaced' });
      const active = packages.filter(pkg => pkg.status === 'SUCCESS');
      expect(active.map(pkg => pkg.tracking).sort(), JSON.stringify(logger.error.mock.calls)).toEqual(split ? ['TRACK44011', 'TRACK44012'] : ['TRACK44011']);
      expect(providerCalls.filter(call => call.operation === 'cancel')).toHaveLength(mode === 'unsent' || mode === 'claimed_before_void' ? 0 : 1);
      const calls = providerCalls.length; await worker.runDueBatch(); expect(providerCalls).toHaveLength(calls);
      if (split) {
        await pool.query("UPDATE wms.outbound_shipments SET status = 'shipped', tracking_number = 'TRACK44011'");
        const shipmentIds = (await pool.query('SELECT id FROM wms.outbound_shipments')).rows.map(row => row.id);
        const dispatchReplay = await repository.materializePhysicalPackage({ legacyWmsShipmentIds: shipmentIds,
          shippingProvider: 'shipstation', providerPhysicalShipmentId: '44011', providerOrderId: '99001',
          trackingNumber: 'TRACK44011', carrier: 'ups', shippedAt: clock.now(), source: 'carrier_tracking_confirmed_dispatch',
          legacyHeaderPolicy: 'aggregate_projection' });
        expect(dispatchReplay.channelCommands).toHaveLength(1);
        expect((await pool.query('SELECT tracking_number FROM oms.channel_fulfillment_pushes WHERE id = $1',
          [dispatchReplay.channelCommands[0].id])).rows).toEqual([{ tracking_number: 'TRACK44011' }]);
        expect((await pool.query(`SELECT item.id FROM wms.physical_shipment_items item
          JOIN wms.order_items order_item ON order_item.id = item.wms_order_item_id
          JOIN oms.oms_order_lines line ON line.id = order_item.oms_order_line_id
          WHERE line.external_line_item_id = 'gid://shopify/LineItem/640003'`)).rowCount).toBe(0);
      }
      expect((await pool.query('SELECT SUM(quantity_shipped)::int AS quantity FROM wms.effective_physical_shipment_items')).rows).toEqual([{ quantity: split ? 2 : 1 }]);
      expect((await pool.query('SELECT id FROM inventory.inventory_transactions')).rowCount).toBe(0);
      await expect(pool.query("UPDATE oms.shopify_label_void_work SET state = 'pending', next_attempt_at = now(), completed_at = NULL")).rejects.toMatchObject({ code: '55000' });
  ...
  it.each(['poll', 'replacement_webhook'])("discovers a missed combined-order void and queues a correction for each exact originating order: %s", async mode => {
    ...
    await pool.query("UPDATE oms.oms_orders SET external_order_id = 'gid://shopify/Order/' || (640000 + id)");
    await pool.query("UPDATE oms.oms_order_lines SET external_line_item_id = 'gid://shopify/LineItem/' || (650000 + id)");
    const items = sources.map((id, index) => ({ lineItemKey: `wms-item-${id}`, quantity: index === 0 ? 12 : 5 }));
    const oldId = await seedAuthorityReadinessLabel(pool, sources[0], { providerLabelId: '460595426', providerOrderId: '790960007',
      trackingNumber: '877510065050', carrierCode: 'fedex', contentsLines: items });
    await pool.query("UPDATE wms.shipping_provider_labels SET normalized_tracking_number = tracking_number, carrier = 'fedex'");
    await pool.query(`INSERT INTO wms.shipping_provider_label_links(shipping_provider_label_id, legacy_wms_shipment_id)
      SELECT label.id, item.shipment_id FROM wms.shipping_provider_labels label CROSS JOIN wms.outbound_shipment_items item`);
    const clock = { now: () => new Date('2099-09-21T20:00:00Z') };
    const logger = { info: vi.fn(), warn: vi.fn(), error: vi.fn() };
    const handler = new PackageAllocationLabelCommercialFulfillmentService({ enabled: true, logger,
      workflow: createPackageAllocationLabelCommercialWorkflow({ pool, clock, logger }),
      labelLinker: { reconcileShipStationLabel: vi.fn().mockResolvedValue({ linksInserted: 0, totalLinks: 0 }) },
      reviewRepository: { record: vi.fn() } });
    const original = { shipmentId: 460595426, orderId: 790960007, orderKey: 'echelon-wms-shp-17763', orderNumber: '63300',
      trackingNumber: '877510065050', carrierCode: 'fedex', serviceCode: 'fedex_ground', shipmentCost: 0,
      isReturnLabel: false, shipDate: '2026-09-21', voidDate: null as string | null, shipmentItems: items.map(item => ({ ...item, sku: 'SAME-SKU' })) };
    ...
    expect((await pool.query('SELECT reason FROM wms.ebay_label_replacement_work WHERE shipping_provider_label_id = $1', [replacementId])).rows)
  ...
  it.each(['shopify', 'ebay'] as const)('conserves split, merge, rollback and duplicate repack events for %s', async provider => {
    const source = await seedCommercialFulfillmentAuthoritySource(pool, `REPACK-${provider}`, 2);
    await pool.query('UPDATE channels.channels SET provider = $1', [provider]);
    await pool.query('UPDATE oms.oms_order_lines SET fulfillment_provider = $1', [provider]);
    if (provider === 'ebay') {
      await pool.query("UPDATE oms.oms_orders SET external_order_id = 'repack-ebay-order'");
      await pool.query("UPDATE oms.oms_order_lines SET external_line_item_id = 'repack-ebay-line'");
    }
    ...
    const handler = new PackageAllocationLabelCommercialFulfillmentService({ enabled: true, logger,
      labelLinker: { reconcileShipStationLabel: vi.fn().mockResolvedValue({ linksInserted: 0, totalLinks: 0 }) },
      reviewRepository: { record: vi.fn() }, workflow: { run: work => workflow.run(async context => {
        const result = await work(context); if (rollback) throw new Error('repack rollback'); return result;
      }) } });
    async function label(providerLabelId: number, quantity: number, providerOrderId = 99001) {
      const items = [{ lineItemKey: `wms-item-${source}`, quantity }];
      const id = await seedAuthorityReadinessLabel(pool, source, { providerLabelId: String(providerLabelId), providerOrderId: String(providerOrderId),
        trackingNumber: `REPACK${providerLabelId}`, carrierCode: 'ups', contentsLines: items, receivedAt: '2026-09-17T12:00:00Z' });
      await pool.query("UPDATE wms.shipping_provider_labels SET carrier = 'ups' WHERE id = $1", [id]);
      await pool.query(`INSERT INTO wms.shipping_provider_label_links(shipping_provider_label_id, legacy_wms_shipment_id)
        SELECT $1, shipment_id FROM wms.outbound_shipment_items WHERE id = $2`, [id, source]);
      return { id, receive: () => handler.process({ shipmentId: providerLabelId, orderId: providerOrderId,
        trackingNumber: `REPACK${providerLabelId}`, isReturnLabel: false, shipmentItems: items }, { shippingProviderLabelId: String(id) } as any) };
    }
    const lifecycleRepository = createShopifyLabelLifecycleRepository(getTestDb());
    const client: ShopifyAdminGraphQLClient = { async request<T>() {
      return { order: { id: 'gid://shopify/Order/640001', fulfillmentsCount: { count: 0 }, fulfillments: [] } } as T;
    } };
    const lifecycle = createShopifyLabelLifecycleService({ repository: lifecycleRepository,
      providerClients: { shopify: async channelId => ({ channelId, connectionId: 987, externalAccountId: 'test.myshopify.com', client }),
        ebay: async () => { throw new Error('No eBay writes in allocation test'); } }, runExclusive: async (_id, work) => work(), clock, logger });
    async function voidLabel(id: number) {
      const original = (await pool.query('SELECT sanitized_payload FROM wms.shipping_provider_label_events WHERE shipping_provider_label_id = $1 ORDER BY id LIMIT 1', [id])).rows[0].sanitized_payload;
      const payload = { ...original, voidDate: '2026-09-17T13:00:00Z' };
      const hash = createHash('sha256').update(canonicalJson({ provider: 'shipstation', ...payload, labelStatus: 'voided' })).digest('hex');
      await pool.query(`INSERT INTO wms.shipping_provider_label_events(shipping_provider_label_id,event_hash,event_type,label_status,tracking_number,provider_occurred_at,received_at,sanitized_payload)
        VALUES ($1,$2,'label_voided','voided',$3,'2026-09-17T13:00:00Z','2026-09-17T13:00:00Z',$4)`, [id, hash, payload.trackingNumber, payload]);
      await pool.query("UPDATE wms.shipping_provider_labels SET label_status = 'voided', last_observed_at = '2026-09-17T13:00:00Z' WHERE id = $1", [id]);
      if (provider === 'shopify') { await lifecycle.observe(id); await lifecycle.runDueBatch(); }
    }
    const original = await label(71001, 2); expect(await original.receive()).toMatchObject({ outcome: 'activated' });
    await voidLabel(original.id);
    // A negative-only transfer cannot commit, even if application validation is bypassed.
    const oldItem = (await pool.query('SELECT id FROM wms.physical_shipment_items')).rows[0].id;
    await expect(pool.query(`INSERT INTO wms.physical_shipment_item_quantity_adjustments
      ...
      VALUES ($1,-2,'provider_label_replacement','11111111-1111-4111-8111-111111111111','invalid-half-repack','integration','negative only',
        '{"replacementBatchId":"11111111-1111-4111-8111-111111111111"}', NOW())`, [oldItem])).rejects.toMatchObject({ code: '23514' });
    const a = await label(71002, 1);
    expect(await a.receive()).toMatchObject({ outcome: 'waiting', reason: 'replacement_repack_incomplete' });
    expect((await pool.query('SELECT id FROM wms.physical_shipment_item_quantity_adjustments')).rowCount).toBe(0);
    const b = await label(71003, 1, 99002);
    rollback = true; await expect(b.receive()).rejects.toThrow('repack rollback'); rollback = false;
    expect((await pool.query('SELECT id FROM wms.physical_shipments')).rowCount).toBe(1);
    expect((await pool.query('SELECT id FROM wms.physical_shipment_item_quantity_adjustments')).rowCount).toBe(0);
    const applied = await Promise.all([a.receive(), b.receive(), a.receive()]);
    for (const result of applied) expect(result, JSON.stringify({ result, logs: logger.error.mock.calls })).toMatchObject({ outcome: 'replaced' });
    expect((await pool.query(`SELECT package.tracking_number, item.quantity_shipped FROM wms.effective_physical_shipment_items item
      JOIN wms.physical_shipments package ON package.id = item.physical_shipment_id ORDER BY package.tracking_number`)).rows)
      .toEqual([{ tracking_number: 'REPACK71002', quantity_shipped: 1 }, { tracking_number: 'REPACK71003', quantity_shipped: 1 }]);
    expect((await pool.query("SELECT COUNT(DISTINCT replacement_batch_id)::int AS batches FROM wms.ebay_label_replacement_work WHERE state = 'applied'")).rows).toEqual([{ batches: 1 }]);
    await expect(pool.query("UPDATE wms.ebay_label_replacement_work SET replacement_batch_id = NULL WHERE state = 'applied'")).rejects.toMatchObject({ code: '55000' });
    if (provider === 'ebay') {
      const amend = vi.fn(async (orderId: string, pkg: EbayShippingFulfillmentRequest, previous: readonly string[], batch?: { packages: readonly EbayShippingFulfillmentRequest[] }) => {
        expect(orderId).toBe('repack-ebay-order'); expect(previous).toEqual(['REPACK71001']);
        expect(pkg.lineItems).toEqual([{ lineItemId: 'repack-ebay-line', quantity: 1 }]);
        expect(batch!.packages.map(item => ({ tracking: item.trackingNumber, lines: item.lineItems })))
          .toEqual(['REPACK71002', 'REPACK71003'].map(tracking => ({ tracking, lines: [{ lineItemId: 'repack-ebay-line', quantity: 1 }] })));
        return { fulfillmentId: `remote-${pkg.trackingNumber}` };
      });
      const push = createFulfillmentPushService(getTestDb(), null, { providerClients: {
        shopify: async () => { throw new Error('Wrong originating channel'); },
        ebay: async channelId => ({ channelId, externalAccountId: 'repack-ebay-account', client: {
    ...
    const merged = await label(71004, 2); expect(await merged.receive()).toMatchObject({ outcome: 'replaced' });
    expect(await merged.receive()).toMatchObject({ outcome: 'replaced' });
    expect((await pool.query('SELECT quantity_shipped FROM wms.effective_physical_shipment_items')).rows).toEqual([{ quantity_shipped: 2 }]);
    expect((await pool.query('SELECT id FROM wms.physical_shipment_item_quantity_adjustments')).rowCount).toBe(3);
    expect((await pool.query('SELECT qty FROM wms.outbound_shipment_items')).rows).toEqual([{ qty: 2 }]);
    expect((await pool.query('SELECT id FROM inventory.inventory_transactions')).rowCount).toBe(0);
  }, 30000);

  it('atomically hands off void discovery and fences independently retryable recovery', async () => {
    const repository = createShipStationLabelReconciliationRepository(getTestDb());
    const now = new Date('2026-09-21T20:00:00Z');
    const current = await repository.readOrCreate(new Date('2026-09-20'), now);
    const claim = (await repository.claim(current, planLabelScanWindow(current, now)!, now))!;
    // Force the second insert to fail: neither first queue row nor watermark may commit.
    await pool.query(`CREATE FUNCTION oms.reject_test_recovery_seed() RETURNS trigger LANGUAGE plpgsql AS $$
      BEGIN IF NEW.provider_label_id = 2 THEN RAISE EXCEPTION 'injected handoff failure'; END IF; RETURN NEW; END $$;
      CREATE TRIGGER reject_test_recovery_seed BEFORE INSERT ON oms.shipstation_label_recovery_work FOR EACH ROW EXECUTE FUNCTION oms.reject_test_recovery_seed()`);
    try {
      await expect(repository.completePage(claim, false, now, [{ providerLabelId: 1, providerOrderId: 101, trackingNumber: 'TRACK1' }, { providerLabelId: 2, providerOrderId: 102, trackingNumber: 'TRACK2' }])).rejects.toThrow('injected handoff failure');
    } finally { await pool.query('DROP TRIGGER reject_test_recovery_seed ON oms.shipstation_label_recovery_work; DROP FUNCTION oms.reject_test_recovery_seed()'); }
    expect((await pool.query('SELECT id FROM oms.shipstation_label_reconciliation_checkpoint WHERE completed_through = $1', [current.completedThrough])).rowCount).toBe(1);
    expect((await pool.query('SELECT * FROM oms.shipstation_label_recovery_work')).rowCount).toBe(0);
    await repository.completePage(claim, false, now, [{ providerLabelId: 1, providerOrderId: 101, trackingNumber: 'TRACK1' }, { providerLabelId: 2, providerOrderId: 102, trackingNumber: 'TRACK2' }]);
    const jobs = await Promise.all([repository.claimOrder(now), repository.claimOrder(now), repository.claimOrder(now)]);
    expect(jobs.filter(Boolean)).toHaveLength(2);
    const first = jobs.find(job => job?.providerLabelId === 1)!;
    expect(first.trackingNumber).toBe('TRACK1');
    const second = jobs.find(job => job?.providerLabelId === 2)!;
    await repository.finishOrder(first, { state: 'pending', code: 'PROVIDER_UNAVAILABLE', nextAttemptAt: new Date(now.getTime() + 600000) }, now);
    await repository.finishOrder(second, { state: 'complete', code: null, nextAttemptAt: null }, now);
    expect(await repository.claimOrder(now)).toBeNull();
    const later = new Date(now.getTime() + 600000);
    const retry = (await repository.claimOrder(later))!; expect(retry).toMatchObject({ providerLabelId: 1, attempts: 2 });
    await expect(repository.finishOrder(first, { state: 'complete', code: null, nextAttemptAt: null }, later)).rejects.toMatchObject({ code: 'SHIPSTATION_LABEL_SCAN_LEASE_LOST' });
    const expired = new Date(later.getTime() + 300001);
    const successor = (await repository.claimOrder(expired))!;
    await expect(repository.renewOrder(retry, expired)).rejects.toMatchObject({ code: 'SHIPSTATION_LABEL_SCAN_LEASE_LOST' });
    await repository.finishOrder(successor, { state: 'review', code: 'SHIPSTATION_ORDER_LABEL_LIMIT', nextAttemptAt: null }, expired);
    expect(await repository.claimOrder(expired)).toBeNull();
    const { getFlowBucketSamples } = await import('../../../oms/flow-waterfall.service');
    expect((await getFlowBucketSamples(getTestDb(), 'SHIPSTATION_LABEL_RECOVERY_BLOCKED')).rows)
      .toMatchObject([{ shipstation_order_id: '101', shipstation_shipment_id: '1', tracking_number: 'TRACK1', state: 'review', last_error_code: 'SHIPSTATION_ORDER_LABEL_LIMIT' }]);
    expect((await pool.query('SELECT provider_label_id::int, attempt_number, outcome FROM oms.shipstation_label_recovery_attempts ORDER BY provider_label_id, attempt_number')).rows)
      .toEqual([{ provider_label_id: 1, attempt_number: 1, outcome: 'pending' }, { provider_label_id: 1, attempt_number: 2, outcome: 'review' }, { provider_label_id: 2, attempt_number: 1, outcome: 'complete' }]);
    await expect(pool.query('DELETE FROM oms.shipstation_label_recovery_attempts')).rejects.toMatchObject({ code: '55000' });
    await expect(pool.query("UPDATE oms.shipstation_label_recovery_work SET state = 'pending', next_attempt_at = NOW() WHERE state = 'review'")).rejects.toMatchObject({ code: '55000' });
  });

  it("fences concurrent void scans, resumes pages after restart, and retains failed windows", async () => {
    const repository = createShipStationLabelReconciliationRepository(getTestDb());
    const now = new Date('2026-09-21T20:00:00Z');
    const initial = new Date('2026-09-20T20:00:00Z');
    const checkpoint = await repository.readOrCreate(initial, now);
    const window = planLabelScanWindow(checkpoint, now)!;
    const claims = await Promise.all([repository.claim(checkpoint, window, now), repository.claim(checkpoint, window, now)]);
    expect(claims.filter(Boolean)).toHaveLength(1);
    const claim = claims.find(value => value !== null)!;
    await repository.completePage(claim, true, now);
    const restarted = createShipStationLabelReconciliationRepository(getTestDb());
    const saved = await restarted.readOrCreate(new Date('2026-09-21T19:00:00Z'), now);
    expect(saved).toMatchObject({ completedThrough: initial, window: { ...window, page: 2 } });
    const next = (await restarted.claim(saved, saved.window!, now))!;
    await expect(repository.completePage(claim, false, now)).rejects.toMatchObject({ code: 'SHIPSTATION_LABEL_SCAN_LEASE_LOST' });
    await restarted.fail(next, 'SHIPSTATION_LABEL_SCAN_FAILED', now);
    const failed = await restarted.readOrCreate(initial, now);
    expect(failed.window).toEqual(saved.window); expect(failed.completedThrough).toEqual(initial);
    const retry = (await restarted.claim(failed, failed.window!, now))!;
    await restarted.completePage(retry, false, now);
    expect(await restarted.readOrCreate(initial, now)).toMatchObject({ completedThrough: window.end, window: null, lastSuccessAt: now });
    expect((await pool.query('SELECT last_error_code, consecutive_failures FROM oms.shipstation_label_reconciliation_checkpoint')).rows)
      .toEqual([{ last_error_code: null, consecutive_failures: 0 }]);
  });

  it("does not let an expired void-scan worker renew or commit a successor's progress", async () => {
    const repository = createShipStationLabelReconciliationRepository(getTestDb());
    const now = new Date('2026-09-21T20:00:00Z');
    const checkpoint = await repository.readOrCreate(new Date('2026-09-20'), now);
    const claim = (await repository.claim(checkpoint, planLabelScanWindow(checkpoint, now)!, now))!;
    const afterExpiry = new Date(now.getTime() + 300001);
    await expect(repository.renew(claim, afterExpiry)).rejects.toMatchObject({ code: 'SHIPSTATION_LABEL_SCAN_LEASE_LOST' });
    const saved = await repository.readOrCreate(now, afterExpiry);
    const successor = (await repository.claim(saved, saved.window!, afterExpiry))!;
    await expect(repository.fail(claim, 'STALE_FAILURE', afterExpiry)).rejects.toMatchObject({ code: 'SHIPSTATION_LABEL_SCAN_LEASE_LOST' });
    await repository.completePage(successor, false, afterExpiry);
  });

...
function sources(): { shopify: CustomerReturnShopifySnapshot; local: CustomerReturnLocalInspectionSnapshot } {
  ...
  const local: CustomerReturnLocalInspectionSnapshot = {
    ...
    lines: [{ omsOrderLineId: 11, externalLineItemId: "101", title: "Fictional product", variantTitle: null, sku: "SAME-SKU", quantity: 4, requiresShipping: true, unitWeightGrams: 12.34 }],
    ...
    packageItems: [0, 1].map(offset => ({ physicalShipmentItemId: 51 + offset, physicalShipmentId: 501 + offset,
      ...
      originalQuantity: 2, effectiveQuantity: 2, status: "shipped", provider: "shipstation", providerPhysicalShipmentId: String(501 + offset), trackingNumber: `TRACK-${501 + offset}`, carrier: "Carrier" })),

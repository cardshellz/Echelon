...
import {
  customerReturnLiveStateSchema, customerReturnLiveLookupInputSchema, customerReturnLiveOrderSchema,
  customerReturnLiveReviewInputSchema, customerReturnLiveReviewSchema,
...
type Operation = "state" | "order" | "review" | "live_state" | "live_order" | "live_review" | "page" | "unknown";
...
export function registerCustomerReturnPreviewRoutes(app: Express, dependencies: CustomerReturnPreviewRouteDependencies = {}): void {
  ...
  const liveService = async () => dependencies.liveService
    ?? (await import("../../infrastructure/customer-return-live.composition")).createCustomerReturnLiveService();
  ...
  function sendError(res: Response, operation: Operation, error: unknown): void {
    const known = error instanceof CustomerReturnPreviewAccessError || error instanceof CustomerReturnPreviewError || error instanceof CustomerReturnLiveError;
  ...
  const handle = (operation: Operation, run: (req: Request) => unknown | Promise<unknown>) => async (req: Request, res: Response): Promise<void> => {
    try {
      if (Object.keys(req.query).length > 0) {
  ...
  app.get(`${API_ROOT}/live`, handle("live_state", async () => customerReturnLiveStateSchema.parse(await (await liveService()).getState())));
  app.post(`${API_ROOT}/live/order`, handle("live_order", async req => {
    const request = input(customerReturnLiveLookupInputSchema, req.body);
    return customerReturnLiveOrderSchema.parse(await (await liveService()).lookup(request));
  }));
  app.post(`${API_ROOT}/live/review`, handle("live_review", async req => {
    const request = input(customerReturnLiveReviewInputSchema, req.body);
...
function apiOperation(path: string): Operation {
  ...
  if (endpoint === "/live") return "live_state";
  if (endpoint === "/live/order") return "live_order";
  if (endpoint === "/live/review") return "live_review";
  return "unknown";
}

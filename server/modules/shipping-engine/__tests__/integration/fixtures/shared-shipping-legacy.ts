export const SHARED_SHIPPING_LEGACY_FIXTURE_SQL = `CREATE SCHEMA shipping; CREATE SCHEMA dropship; CREATE SCHEMA warehouse; CREATE SCHEMA catalog;
      ...
      CREATE TABLE shipping.box_catalog(id integer GENERATED ALWAYS AS IDENTITY PRIMARY KEY,code text UNIQUE,name text,kind text,
        ...
        cost_cents integer,fill_factor_bps integer,is_active boolean);
      ...
      CREATE TABLE shipping.channel_policies(id integer PRIMARY KEY,status text,purpose text);
      CREATE TABLE shipping.channel_policy_routes(policy_id integer,rate_book_id integer);

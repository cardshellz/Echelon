...
import type { InventoryTrackingHistorySummary } from "@shared/catalog/inventory-tracking-history";
...
export type BulkInventoryTrackingSelection = Array<{
  ...
  snapshot: {
    ...
    transitions: Array<{
      ...
      history?: { snapshotHash: string; summary: InventoryTrackingHistorySummary };
    }>;
  } | null;
}>;
...
const blockerMessages: Record<string, string> = {
  ...
  quantity_ledger: "managed quantity ledger is active; stopping its balances requires a ledger transition",
  build_reservations: "active build reservations", frozen_locations: "a location is frozen for counting",
  replenishment: "unfinished replenishment work",
};

export function buildBulkInventoryTrackingPreview(selection: BulkInventoryTrackingSelection, next: boolean, disposition?: "retain_history") {
  const products: BulkInventoryTrackingProduct[] = selection.map(({ productId, snapshot }) => {
    ...
    return { productId, name: product.name, sku: product.sku, currentDefault: product.inventoryTrackingDefault,
      ...
      blockers, ...(disposition === "retain_history" ? { history: transitions.flatMap(({ variant, history }) => history
        && (history.summary.levelCount > 0 || history.summary.lotCount > 0)
        ? [{ variantId: variant.id, sku: variant.sku, snapshotHash: history.snapshotHash, summary: history.summary }] : []) } : {}) };
  });
  ...
  const evidence = selection.map(({ productId, snapshot }) => ({ productId,
    ...
    variants: snapshot?.transitions.map(({ variant, effective, blockers, evidence, history }) => ({ id: variant.id,
      ...
      history: history ? { snapshotHash: history.snapshotHash, summary: history.summary } : undefined })) ?? [],
  }));
  const previewHash = createHash("sha256").update(canonicalJson({ version: 3, next, disposition, evidence })).digest("hex");
  return bulkInventoryTrackingPreviewSchema.parse({ previewHash, inventoryTrackingDefault: next, stockDisposition: disposition, products });
}

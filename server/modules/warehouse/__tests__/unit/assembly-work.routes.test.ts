...
describe("assembly work HTTP validation and identity", () => {
  ...
  it("authenticates and validates packing handoff, with session identity only", async () => {
    ...
    authenticated = false;
    expect((await post("/1/packing-ready", command)).status).toBe(401);
    authenticated = true;
    for (const invalid of [{ ...command, actorId: "admin" }, { ...command, expectedVersion: 0 }, { ...command, confirmReadyForPacking: false }])
      expect((await post("/1/packing-ready", invalid)).status).toBe(400);
    expect(packing.ready).not.toHaveBeenCalled();
    packing.ready.mockRejectedValue(new WarehouseWorkError("WORK_PACKING_BLOCKED", "Another line is incomplete", 409));
    expect((await post("/1/packing-ready", command)).status).toBe(409);
    expect(packing.ready).toHaveBeenCalledWith("session-user", "1", command);
  });

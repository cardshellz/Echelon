...
export function registerDropshipAdminOrderOpsRoutes(
  ...
  app.post(
    "/api/dropship/admin/order-intake/:intakeId/retry-wms-sync",
    requirePermission("dropship", "manage_operations"),
    async (req, res) => {
      try {
...
function statusForDropshipOrderOpsError(code: string): number {
  switch (code) {
    ...
    case "DROPSHIP_ORDER_OPS_WMS_SYNC_INVALID_INPUT":
    ...
    case "DROPSHIP_ORDER_OPS_WMS_SYNC_NOT_CONFIGURED":
    case "DROPSHIP_ORDER_OPS_WMS_SYNC_RETRY_ENQUEUE_FAILED":
      return 503;
    default:
      return 500;
  }

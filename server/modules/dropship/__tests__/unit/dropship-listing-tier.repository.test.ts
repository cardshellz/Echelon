...
function fakePool(scenario: Scenario = {}) {
  ...
  const query = vi.fn(async (sql: string, values?: unknown[]) => {
    ...
    if (text.startsWith("UPDATE dropship.dropship_vendor_listing_tier_holds SET evaluated_at")) {
      return { rows: [{ ...(scenario.existing as Record<string, unknown>), evaluated_at: values?.[1] }] };
    }
    if (text.startsWith("INSERT INTO dropship.dropship_vendor_listing_tier_holds")) {
      ...
      return { rows: [holdRow({
        ...
        applied: false, detail: values?.[2], evaluated_at: values?.[3], applied_at: null,
      })] };
    }
...
describe("PgDropshipListingTierRepository", () => {
  it("lists active and paused vendors with their recorded tier hold, oldest first", async () => {
    const { pool, params } = fakePool({ vendors: [
      { vendor_id: 10, status: "active", held_tiers: ["case"], revision: 3, applied: true, detail: "held case", evaluated_at: NOW, applied_at: NOW },
      { vendor_id: 11, status: "paused", held_tiers: null, revision: null, applied: null, detail: null, evaluated_at: null, applied_at: null },
    ] });

    const vendors = await new PgDropshipListingTierRepository(pool).listVendorsForReview({ limit: 50 });

    expect(vendors).toEqual([
      { vendorId: 10, status: "active", tierHold: { vendorId: 10, heldTiers: ["case"], revision: 3, applied: true, detail: "held case", evaluatedAt: NOW, appliedAt: NOW } },
  ...
  it("records a changed set of held tiers with a bumped revision and an audit row in one transaction", async () => {
    ...
    const result = await new PgDropshipListingTierRepository(pool).recordHeldTiers({
      vendorId: 10, heldTiers: ["case", "pack"], detail: "pack: off sale; case: off sale", now: NOW,
    });

    expect(result).toEqual({
      changed: true,
      record: { vendorId: 10, heldTiers: ["pack", "case"], revision: 4, applied: false, detail: "pack: off sale; case: off sale", evaluatedAt: NOW, appliedAt: null },
    });
    ...
    expect(params[insertIndex]).toEqual([10, ["pack", "case"], "pack: off sale; case: off sale", NOW]);
    ...
    expect(JSON.parse(String(params[auditIndex]?.[3]))).toEqual({
      before: { heldTiers: [], revision: 3 },
      after: { heldTiers: ["pack", "case"], revision: 4 },
      detail: "pack: off sale; case: off sale",
    });
    expect(release).toHaveBeenCalledOnce();
  });

  it("only refreshes the evaluation time when the held tiers did not change", async () => {
    const { pool, statements } = fakePool({ existing: holdRow({ held_tiers: ["case"], revision: 3 }) });

    const result = await new PgDropshipListingTierRepository(pool).recordHeldTiers({ vendorId: 10, heldTiers: ["case"], detail: "x", now: NOW });

    expect(result).toEqual({ changed: false, record: expect.objectContaining({ heldTiers: ["case"], revision: 3, applied: true, evaluatedAt: NOW }) });
  ...
  it("creates the first row at revision one for a vendor without one", async () => {
    ...
    expect(result).toEqual({ changed: true, record: expect.objectContaining({ heldTiers: [], revision: 1, applied: false }) });
  });
  ...
  it("refuses stored tiers or revisions it does not recognise", async () => {
    const { pool } = fakePool({ vendors: [
      { vendor_id: 10, status: "active", held_tiers: ["pallet"], revision: 3, applied: true, detail: null, evaluated_at: NOW, applied_at: NOW },
    ] });

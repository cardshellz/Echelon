...
describe("DropshipSelectionAtpService", () => {
  ...
  it("returns catalog rows with exposure status and builds de-duped facets", async () => {
    const repository = new FakeDropshipSelectionAtpRepository();
    ...
    expect(result.total).toBe(2);
    expect(result.rows.map((row) => row.productVariantId)).toEqual([20, 21]);
    expect(result.rows[1].selectionDecision.reason).toBe("not_exposed_by_admin");
    expect(result.facets.categories).toEqual([
      { category: "Other", label: "Other", rowCount: 1 },
      { category: "Supplies", label: "Supplies", rowCount: 1 },
    ]);
    expect(result.facets.productLines).toContainEqual({
      productLineIds: [30, 31],
      label: "Trading Card Supplies",
      rowCount: 1,
    });
    expect(result.facets.productLines).toContainEqual({
      productLineIds: [40],

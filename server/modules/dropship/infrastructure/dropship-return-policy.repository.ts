...
export class PgDropshipReturnPolicyRepository implements DropshipReturnPolicyRepository {
  ...
  async resolveReturnFees(input: {
    ...
    for (const row of result.rows) {
      ...
      if (record.feeType === "restocking_fee" && !resolved.restockingFee) {
        ...
        record.feeType === "processing_fee" &&
        !resolved.processingFee
      ) {
        resolved.processingFee = record;
      } else if (
        record.feeType === "return_shipping_fee" &&
        !resolved.returnShippingFee
      ) {
  ...
  async resolveDefaultReturnFees(input: {
    ...
    const result = await this.dbPool.query<FeeRow & { specificity: number }>(
      `SELECT f.*, scope.specificity
       FROM dropship.dropship_return_fee_schedule f
       JOIN (VALUES (1, 'global'), (2, 'store'), (3, 'vendor'), (4, 'vendor_store')) AS scope(specificity, scope_name) ON true
       WHERE f.is_active = true AND f.is_default = true
         AND f.effective_from <= $3 AND (f.effective_to IS NULL OR f.effective_to > $3)
         AND CASE scope.scope_name
           WHEN 'global' THEN f.vendor_id IS NULL AND f.store_connection_id IS NULL
           WHEN 'store' THEN f.vendor_id IS NULL AND f.store_connection_id = $2
           WHEN 'vendor' THEN f.vendor_id = $1 AND f.store_connection_id IS NULL
    ...
    return mapResolvedFees(result.rows);
  }
  ...
  async createPolicyVersion(
    ...
    try {
      ...
      if (command.idempotentReplay) {
        const policy = await loadPolicyByIdWithClient(
          client,
          parseEntityId(command.entityId),
        );
      ...
      const policy = mapPolicyRow(
        requiredRow(
          inserted.rows[0],
          "Dropship return policy insert returned no row.",
        ),
      );
  ...
  async createFeeVersion(
    ...
    try {
      ...
      if (command.idempotentReplay) {
        const fee = await loadFeeByIdWithClient(
          client,
          parseEntityId(command.entityId),
        );
        await client.query("COMMIT");
      ...
      if (input.isDefault) {
        await client.query(
          ...
          [input.now, input.feeType, input.vendorId, input.storeConnectionId],
        );
      }
      const inserted = await client.query<FeeRow>(
        `INSERT INTO dropship.dropship_return_fee_schedule
          (version, fee_type, fault_category, amount_type, amount,
           vendor_id, store_connection_id, priority, is_active, is_default,
           effective_from, effective_to, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, true, $9, $10, NULL, $11, $11)
         RETURNING *`,
        [
          version,
          input.feeType,
      ...
      const fee = mapFeeRow(
        requiredRow(
          inserted.rows[0],
          "Dropship return fee insert returned no row.",
        ),
      );
      await completeAdminCommand(
        client,
      ...
      if (!row) {
        ...
        if (!existing.rows[0]) {
          throw new DropshipError(
            "DROPSHIP_RETURN_POLICY_NOT_FOUND",
            "Dropship return policy was not found.",
            {
              policyId: input.policyId,
            },
          );
  ...
  async deactivateFee(input: {
    ...
    try {
      ...
      if (!row) {
        ...
        if (!existing.rows[0]) {
          throw new DropshipError(
            "DROPSHIP_RETURN_FEE_NOT_FOUND",
            "Dropship return fee schedule row was not found.",
            {
              feeId: input.feeId,
            },
          );
  ...
  private async findPolicyReplayAfterUniqueConflict(
    ...
    try {
      ...
      if (
        !row ||
        row.command_type !== "return_policy_version_created" ||
  ...
  private async findFeeReplayAfterUniqueConflict(
    ...
    try {
      ...
      if (
        !row ||
        row.command_type !== "return_fee_version_created" ||
        row.request_hash !== expectedHash ||
        !row.entity_id
      ) {
        await client.query("COMMIT");
...
async function nextPolicyVersion(
  ...
  return Number(
    requiredRow(
      result.rows[0],
      "Dropship return policy version query returned no row.",
    ).next_version,
  );
}
...
async function nextFeeVersion(
  ...
  return Number(
    requiredRow(
      result.rows[0],
      "Dropship return fee version query returned no row.",
    ).next_version,
  );
}

async function loadPolicyByIdWithClient(
  client: PoolClient,
  policyId: number,
): Promise<DropshipReturnPolicyVersionRecord> {
...
async function loadFeeByIdWithClient(
  client: PoolClient,
  feeId: number,
): Promise<DropshipReturnFeeScheduleRecord> {
...
async function claimAdminCommand(
  ...
  const row = requiredRow(
    existing.rows[0],
    "Dropship admin config command row was not found after conflict.",
  );
  if (row.command_type !== commandType || row.request_hash !== requestHash) {
    throw new DropshipError(
      ...
      "Dropship return policy idempotency key was reused with a different request.",
      {
        commandType,
        idempotencyKey: input.idempotencyKey,
        requestHashMatches: row.request_hash === requestHash,
      },
    );
  }
  if (!row.entity_id) {
...
function mapResolvedFees(rows: readonly FeeRow[]): DropshipResolvedReturnFees {
  const resolved: DropshipResolvedReturnFees = {
    restockingFee: null,
    processingFee: null,
    returnShippingFee: null,
  ...
  for (const row of rows) {
    ...
    if (record.feeType === "restocking_fee" && !resolved.restockingFee)
      resolved.restockingFee = record;
    if (record.feeType === "processing_fee" && !resolved.processingFee)
      resolved.processingFee = record;
    if (record.feeType === "return_shipping_fee" && !resolved.returnShippingFee)
      resolved.returnShippingFee = record;
  }
  return resolved;
}

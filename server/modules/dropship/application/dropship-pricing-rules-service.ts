...
export class DropshipPricingRulesService {
  ...
  async targetsForMember(memberId: string, storeId: unknown, input: unknown): Promise<{ total: number; rows: Array<{ id: string; name: string }> }> {
    ...
    return this.execute(memberId, storeId, async (tx) => {
      const candidates = await loadSelectedCandidates(tx, this.deps.clock.now());
      ...
      return { total: unique.length, rows: unique.slice(parsed.page * PRICING_REVIEW_PAGE_SIZE, (parsed.page + 1) * PRICING_REVIEW_PAGE_SIZE) };
    });
  }
...
async function loadSelectedCandidates(tx: PricingRulesTransaction, now: Date): Promise<DropshipListingCatalogCandidate[]> {
  ...
  for (;;) {
    ...
    if (selected.length > MAX_PRICING_REVIEW_ITEMS) throw new DropshipError("DROPSHIP_PRICING_REVIEW_TOO_LARGE",

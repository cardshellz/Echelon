...
export type DropshipReturnFeeFaultCategory = z.infer<typeof dropshipReturnFeeFaultCategorySchema>;

export const dropshipReturnFeeAmountTypeSchema = z.enum(["flat_cents", "percent"]);
export type DropshipReturnFeeAmountType = z.infer<typeof dropshipReturnFeeAmountTypeSchema>;

const positiveIdSchema = z.number().int().positive();

export const resolveReturnPolicyInputSchema = z.object({
  vendorId: positiveIdSchema.nullable().optional(),
  storeConnectionId: positiveIdSchema.nullable().optional(),
  at: z.coerce.date().optional(),
}).strict();
export type ResolveReturnPolicyInput = z.infer<typeof resolveReturnPolicyInputSchema>;

export const resolveReturnFeesInputSchema = z.object({
  vendorId: positiveIdSchema.nullable().optional(),
  storeConnectionId: positiveIdSchema.nullable().optional(),
  faultCategory: dropshipReturnFeeFaultCategorySchema,
  at: z.coerce.date().optional(),
}).strict();
export type ResolveReturnFeesInput = z.infer<typeof resolveReturnFeesInputSchema>;
...
const createReturnPolicyVersionInputSchema = z.object({
  ...
  if (input.storeConnectionId != null && input.vendorId == null) {
    ctx.addIssue({
      ...
      message: "store-scoped policies require a vendorId (store connections belong to vendors).",
    });
  }
});
export type CreateReturnPolicyVersionInput = z.infer<typeof createReturnPolicyVersionInputSchema>;

const createReturnFeeVersionInputSchema = z.object({
  ...
  if (input.storeConnectionId != null && input.vendorId == null) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["storeConnectionId"],
      message: "store-scoped fee rows require a vendorId (store connections belong to vendors).",
    });
  }
...
export type CreateReturnFeeVersionInput = z.infer<typeof createReturnFeeVersionInputSchema>;

const deactivateReturnPolicyInputSchema = z.object({
...
const deactivateReturnFeeInputSchema = z.object({
...
const listReturnPoliciesInputSchema = z.object({
  vendorId: positiveIdSchema.nullable().optional(),
  storeConnectionId: positiveIdSchema.nullable().optional(),
  includeInactive: z.boolean().default(false),
}).strict();

const listReturnFeesInputSchema = z.object({
  vendorId: positiveIdSchema.nullable().optional(),
  storeConnectionId: positiveIdSchema.nullable().optional(),
...
export class DropshipReturnPolicyService {
   ...
   */
  async resolveReturnPolicy(input: unknown): Promise<DropshipReturnPolicyVersionRecord | null> {
    const parsed = parsePolicyInput(resolveReturnPolicyInputSchema, input, "DROPSHIP_RETURN_POLICY_INVALID_INPUT");
  ...
  async resolveReturnFees(input: unknown): Promise<DropshipResolvedReturnFees> {
    const parsed = parsePolicyInput(resolveReturnFeesInputSchema, input, "DROPSHIP_RETURN_FEE_INVALID_INPUT");
  ...
  async listPolicies(input: unknown = {}): Promise<DropshipReturnPolicyVersionRecord[]> {
    const parsed = parsePolicyInput(listReturnPoliciesInputSchema, input, "DROPSHIP_RETURN_POLICY_INVALID_INPUT");
    return this.deps.repository.listPolicies(parsed);
  }

  async listFees(input: unknown = {}): Promise<DropshipReturnFeeScheduleRecord[]> {
    const parsed = parsePolicyInput(listReturnFeesInputSchema, input, "DROPSHIP_RETURN_FEE_INVALID_INPUT");
   ...
   */
  async createPolicyVersion(input: unknown): Promise<DropshipReturnPolicyMutationResult> {
    const parsed = parsePolicyInput(createReturnPolicyVersionInputSchema, input, "DROPSHIP_RETURN_POLICY_INVALID_INPUT");
  ...
  async createFeeVersion(input: unknown): Promise<DropshipReturnFeeMutationResult> {
    const parsed = parsePolicyInput(createReturnFeeVersionInputSchema, input, "DROPSHIP_RETURN_FEE_INVALID_INPUT");
    const now = this.deps.clock.now();
  ...
  async deactivatePolicy(input: unknown): Promise<DropshipReturnPolicyMutationResult> {
    const parsed = parsePolicyInput(deactivateReturnPolicyInputSchema, input, "DROPSHIP_RETURN_POLICY_INVALID_INPUT");
    ...
    if (!result.idempotentReplay) {
      this.deps.logger.info({
        ...
        context: { policyId: result.policy.policyId, idempotencyKey: parsed.idempotencyKey },
      });
    }
    return result;
  }

  async deactivateFee(input: unknown): Promise<DropshipReturnFeeMutationResult> {
    const parsed = parsePolicyInput(deactivateReturnFeeInputSchema, input, "DROPSHIP_RETURN_FEE_INVALID_INPUT");
    ...
    if (!result.idempotentReplay) {
      this.deps.logger.info({
        ...
        context: { feeId: result.fee.feeId, idempotencyKey: parsed.idempotencyKey },
      });
    }
    return result;
  }
...
function parsePolicyInput<T>(schema: z.ZodType<T, z.ZodTypeDef, unknown>, input: unknown, code: string): T {
  const result = schema.safeParse(input);
  if (!result.success) {
    throw new DropshipError(code, "Dropship return policy input failed validation.", {
...
function logPolicyEvent(level: "info" | "warn" | "error", event: DropshipLogEvent): void {

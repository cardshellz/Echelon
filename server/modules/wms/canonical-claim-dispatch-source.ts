 ...
 */
export class WmsCanonicalClaimDispatchSourceOwner implements CanonicalClaimDispatchSourceOwner {
  async lockDispatchSource(input: Parameters<CanonicalClaimDispatchSourceOwner["lockDispatchSource"]>[0]):
    ...
    requireFact(order.id === command.orderId && order.warehouse_id === command.warehouseId,
      "WMS_DISPATCH_IDENTITY_MISMATCH", "Order does not belong to the exact dispatch warehouse");
    ...
    requireFact(source.id === command.sourceShipmentItemId && source.shipment_id === command.outboundShipmentId
      ...
      && source.from_location_id === command.warehouseLocationId,
    "WMS_DISPATCH_IDENTITY_MISMATCH", "Source identity or persisted source bin differs from the exact command");
    ...
    if (physicalRows.rows.length === 0) {
      requireFact(command.physicalShipmentId === null && command.physicalShipmentItemId === null,
        "WMS_DISPATCH_PHYSICAL_CONFLICT", "Command physical identity has no exact persisted source linkage");
    } else {
      const identity = parse(physicalIdentitySchema, physicalRows.rows[0], "physicalIdentity");
      requireFact(identity.id === command.physicalShipmentItemId && identity.physical_shipment_id === command.physicalShipmentId,
    ...
    const result = parse(canonicalClaimDispatchEvidenceSchema.shape.source, {
      ...
      physicalShipmentId: command.physicalShipmentId, physicalShipmentItemId: command.physicalShipmentItemId,

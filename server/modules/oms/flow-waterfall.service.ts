...
export async function getFlowBucketSamples(
  ...
  return await db.transaction(async (tx: any) => {
    ...
    const BUCKET: Record<string, any> = {
      WEBHOOK_INBOX_FAILED: sql`SELECT COALESCE(payload->>'name', payload->>'order_number', payload->>'order_id') AS order_number, provider, topic, status, attempts, last_error, COALESCE(processed_at, last_attempt_at, first_received_at) AS at, id AS inbox_id FROM oms.webhook_inbox WHERE status IN ('failed','dead') ORDER BY COALESCE(processed_at, last_attempt_at, first_received_at) DESC NULLS LAST LIMIT 50`,

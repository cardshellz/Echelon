...
export class WmsSyncService {
  ...
  private async syncOmsOrderToWmsInternal(
    ...
    try {
      ...
      // blocks the sync.
      // A Dropship order ships from the warehouse acceptance pinned (the store's
      // default, already validated against the Dropship OMS assignments), never
      // from wherever the generic router would send it.

    ...
    await inTransaction(this.connectionPool, async (client) => {
      ...
      await client.query(
        `INSERT INTO inventory.inventory_publication_readbacks (
           ...
           channel_connection_id_snapshot, provider_scope_type_snapshot,
        ...
        [target.publicationTargetId, target.productVariantId, observedQuantity,
          ...
          target.channelConnectionId, target.providerScopeType, target.externalScopeId,
          target.publicationTargetRevision, observedAt.toISOString()],
      );
    });
  }
...
async function loadTargets(client: PoolClient): Promise<PublicationReadbackTarget[]> {
  const rows = (await client.query<Record<string, unknown>>(
    `SELECT target.id AS publication_target_id, target.revision AS publication_target_revision,
            target.channel_id,
            target.channel_connection_id, channel.provider AS provider_key,
  ...
  return rows.map((row) => ({
    ...
    channelConnectionId: positiveInteger(row.channel_connection_id, "channelConnectionId"),

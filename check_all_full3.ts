...
async function run() {
  try {
    ...
    for(const t of tables) {
      try {
        const cRes = await db.execute(sql.raw("SELECT COUNT(*) FROM public." + t));
        const count = parseInt(cRes.rows[0].count);
        output.push({ table: t, rows: count });
      } catch(e) { }
    }
    fs.writeFileSync('c:/tmp/all_tables_utf8.json', JSON.stringify(output, null, 2), 'utf8');
    process.exit(0);
  } catch(err) {
    console.error(err);
    process.exit(1);
